...
+ (void)initialize {
    if (self == [BlockAlertView class]) {
        //背景image
        background = [[QMImageFactory commonBackgroundImage] retain];
        
        backgroundlandscape = [UIImage imageNamed:kAlertViewBackgroundLandscape];
        //kAlertViewBackgroundCapWidth and  kAlertViewBackgroundCapHeight is 5
        ...
        //font=[UIFont boldSystemFontOfSize:<#(CGFloat)#>] 18 15 16
...
//        UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(kAlertViewBorderHorizontal, _height, frame.size.width-kAlertViewBorderHorizontal*2, size.height)];
        UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(kAlertViewBorderHorizontal, 0, frame.size.width-kAlertViewBorderHorizontal*2, 40)];
    ...
    if (_message)
    {
        ...
        UILabel *labelView = [[UILabel alloc] initWithFrame:CGRectMake(kAlertViewBorderHorizontal, _height + 7, frame.size.width-kAlertViewBorderHorizontal*2, size.height)];
...
- (void)setupDisplay
{
    ...
    frame.origin.x = 8;
    frame.size.width = CGRectGetWidth([UIScreen mainScreen].bounds) - 2 * 8;
...
- (void)show
{
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++) {
        ...
        if (width == maxWidth || isSecondButton) {
            UIImage *separatorImage = [QMImageFactory commonHorizontalLineImage];
            ...
            UIImage *separatorImage = [QMImageFactory commonHorizontalLineImage];
            UIImageView *separatorImageView = [[UIImageView alloc] initWithImage:separatorImage];
            separatorImageView.frame = CGRectMake(maxHalfWidth - kAlertButtonHeight / 2.0f , _height + kAlertButtonHeight / 2.0f, kAlertButtonHeight - 1, 1);
            separatorImageView.transform = CGAffineTransformMakeRotation(M_PI_2);
            [_view addSubview:separatorImageView];
        }
...
#pragma clan diagnostic pop
    ...
    // top bg
    UIImageView *topModelBg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_view.frame), 40.0f)];
    topModelBg.image = [QMImageFactory commonBackgroundImageTopPartPressed];
    topModelBg.contentMode = UIViewContentModeScaleToFill;
    topModelBg.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [_view insertSubview:topModelBg aboveSubview:modalBackground];
    [topModelBg release];
