...
-(void)initiallizeBanner
{
    
     NSString* str=@"1111111111111111111111111111111!!!!!!!!!!!!!";
    _scrollViewBanner=[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, kBanneerHeight)];
    CGFloat scrollViewWidth=_scrollViewBanner.frame.size.width;
    CGFloat width1=[str boundingRectWithSize:CGSizeMake(CGFLOAT_MAX,kBanneerHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:18]} context:nil].size.width;
//    NSLog(@"%f",width1);
   _bannerLabel=[[UILabel alloc] initWithFrame:CGRectMake(10+scrollViewWidth,0,width1,kBanneerHeight)];
    _bannerLabel.adjustsFontSizeToFitWidth=YES;
    _scrollViewBanner.alwaysBounceHorizontal=YES;
    _bannerLabel.textAlignment=NSTextAlignmentLeft;
   
    _bannerLabel.textColor=[UIColor purpleColor];
        [_scrollViewBanner setContentSize:CGSizeMake(self.frame.size.width*3, kBanneerHeight)];
    _bannerLabel.text=str;

    [_scrollViewBanner addSubview:_bannerLabel];
    [_scrollViewBanner setContentOffset:CGPointMake(10, 0)];
    
    [self addSubview:_scrollViewBanner];
    
    NSTimer* timer=[NSTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(bannerSlider) userInfo:nil repeats:YES];
    [timer fire];
...
-(void)bannerSlider
{
    float bannerX=_scrollViewBanner.contentOffset.x;
    
    if (bannerX>=(self.frame.size.width+10+_bannerLabel.frame.size.width)) {
        [_scrollViewBanner setContentOffset:CGPointMake(10, 0)];
//        
...
//        [UIView commitAnimations];
    ...
    else
    {
        ...
        [UIView beginAnimations:nil context:nil];
        bannerX+=1;
        [_scrollViewBanner setContentOffset:CGPointMake(bannerX, 0)];
        [UIView commitAnimations];
    ...
    if([aImageUrls count] > 0) {
        ...
        for (int i = 0; i < [aImageUrls count]; i++) {
            ...
            if([[aImageUrls objectAtIndex:i] isKindOfClass:[UIImage class]]) {
                ...
                // 先查看cache中是否有
                UIImage *image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:[imageUrl path]];
                if (nil != image) {
                    ...
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
                        ...
                        // save image
                        [[SDImageCache sharedImageCache] storeImage:[UIImage imageWithData:imageData] forKey:[imageUrl path] toDisk:YES];
