...
//    }
    if ([VENTouchLock shouldUseTouchID] || [[QMAccountUtil sharedInstance] isUserUsingRectPwd]) {
        [self showTouchID];
    }
    else {
        [self showPasscodeAnimated:animated];
...
- (void)appWillEnterForeground {
    if (!self.presentedViewController) {
        ...
        for (UIViewController *viewController in self.navigationController.viewControllers) {
            if (![viewController isKindOfClass:[VENTouchLockSplashViewController class]] && ![viewController isKindOfClass:[QMInputPasswordViewController class]] && ![viewController isKindOfClass:[QMGesturePasswordViewController class]]) {
                canShow = NO;
                break;
            }
        }
