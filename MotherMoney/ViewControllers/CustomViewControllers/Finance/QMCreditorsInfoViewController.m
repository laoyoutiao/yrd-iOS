...
//详情界面
@interface QMCreditorsInfoViewController ()<UICollectionViewDataSource, UICollectionViewDelegate, UIAlertViewDelegate, QMOpenChannelAccoutViewControllerDelegate>

@end

@implementation QMCreditorsInfoViewController {
    QMCreditorsInfo *mProductInfo;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 获取产品详情
    [[NetServiceManager sharedInstance] getCreditorsDetailWithProductId:mProductInfo.product_id
                                                             delegate:self
                                                              success:^(id responseObject) {
                                                                  // 更新产品信息
                                                                  if ([responseObject isKindOfClass:[NSDictionary class]]) {
                                                                      NSDictionary *dict = [responseObject objectForKey:@"trproduct"];
                                                                      if (QM_IS_DICT_NIL(dict)) {
                                                                          return;
                                                                      }
                                                                      NSLog(@"%@",dict);
                                                                      QMCreditorsInfo *productInfo = [[QMCreditorsInfo alloc] 	initWithDictionary:dict];
       ...
       shareUrl:(NSString *)shareUrl
{
    ...
    //        // 提示用户分享成功
    //    }];
    [QMUMTookKitManager shareTo:platform title:nil content:content image:image shareUrl:shareUrl presentedController:self];
}
...
- (void)setUpBottomBar {
    ...
    if ([mProductInfo.productChannelId isEqualToString:QM_DEFAULT_CHANNEL_ID]) { // 钱宝宝
        ...
        label.text = [NSString stringWithFormat:@"剩余份额%@份", [NSNumber numberWithInteger:[mProductInfo.remainingNum integerValue]]];
...
- (void)buyProductBtnClicked:(UIButton *)btn {
    ...
    [[NetServiceManager sharedInstance] checkProductCanBuyWithProductId:mProductInfo.product_id_real
                                                               delegate:self
                                                                success:^(id responseObject) {
                                                                    // 可以购买
...
- (void)gotoGetProductPage {
    ...
    if ([mProductInfo.productChannelId isEqualToString:QM_DEFAULT_CHANNEL_ID]) {
        QMBuyProductInputMoneyViewControllerV2 *con = [[QMBuyProductInputMoneyViewControllerV2 alloc] initViewControllerWithCreditorsInfo:mProductInfo];
        [self.navigationController pushViewController:con animated:YES];
    }else if ([mProductInfo.productChannelId isEqualToString:@"1"]) {
        QMBuyProductInputMoneyViewController *con = [[QMBuyProductInputMoneyViewController alloc] initViewControllerWithCreditorsInfo:mProductInfo];
        [self.navigationController pushViewController:con animated:YES];
    }
}
...
- (void)calculateEarningsBtnClicked:(UIButton *)btn {
    ...
    QMCalculatorView *calculator = [[QMCalculatorView alloc] initWithFrame:keyWindow.bounds andCreditorsInfo:mProductInfo];
    [calculator show];
}
...
- (void)viewProductDetailInfoBtnClicked:(UIButton *)btn {
    ...
    [[NetServiceManager sharedInstance] getDetailProductInfoWithProductId:mProductInfo.product_id_real
                                                                 delegate:self
                                                                  success:^(id responseObject) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == QMProductInfoTableRow_Abstract) { // 摘要信息，预期年化率
        ...
        [cell configureCellWithCreditorsInfo:mProductInfo ViewConrtoller:self];
        return cell;
        ...
        [cell configureCellWithCreditorsInfo:mProductInfo];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == QMProductInfoTableRow_Abstract) {
        return CGSizeMake(CGRectGetWidth(collectionView.frame) - insets.left - insets.right, [QMProductInfoAbstractCell getCellHeightForCreditorsInfo:mProductInfo]);
    }else if (indexPath.row == QMProductInfoTableRow_Amount) {
        return CGSizeMake(CGRectGetWidth(collectionView.frame) - insets.left - insets.right, [QMProductInfoNumberCell getCellHeightForCreditorsInfo:mProductInfo]);
    }else if (indexPath.row == QMProductInfoTableRow_Action) {
        return CGSizeMake(CGRectGetWidth(collectionView.frame) - insets.left - insets.right, [QMProductInfoActionCell getCellHeightForCreditorsInfo:mProductInfo]);
    }else if (indexPath.item == QMProductInfoTableRow_ProductDescription) {
        return CGSizeMake(CGRectGetWidth(collectionView.frame) - insets.left - insets.right, [QMProductDescriptionCell getCellHeightForCreditorsInfo:mProductInfo]);
    }else {
        return CGSizeMake(CGRectGetWidth(collectionView.frame) - insets.left - insets.right, [QMProductInfoItemCell getCellHeightForCreditorsInfo:mProductInfo]);
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == QMProductInfoTableRow_Introduction)
    {
        ...
        NSString* string2=[string1 stringByAppendingString:mProductInfo.product_id_real];
...
- (void)showContractTemplate {
    [[NetServiceManager sharedInstance] getContractTemplateForProduct:mProductInfo.product_id_real
                                                             delegate:self
                                                              success:^(id responseObject) {
                                                                  if ([responseObject isKindOfClass:[NSDictionary class]]) {
